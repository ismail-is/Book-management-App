...
export default function Remove({del,cl,data, setGetLocal,getLocal }) {

    const Delete = async(item)=>{
        const newValue = getLocal.filter((i)=>i.id!=item.id)
        console.log(newValue,'nae')
        setGetLocal(newValue)
        localStorage.setItem("Fav",JSON.stringify(newValue));
        await cl()
    }
  return ( 
    <Card sx={style}>
      ...
      <CardContent>
        ...
        <Typography variant="body2">
         <h6>ARE SURE YOU WANT TO REMOVE {data.BookName} BOOK FROM FAVOURITE  ?</h6>
        </Typography>
      </CardContent>
      <CardActions>
